...
## `payment_method_id` (catálogo `payment_methods`)
`orders.payment_method_id` (nullable FK → `payment_methods.id`, `SET NULL`) — forma de pagamento do pedido na Bling. Catálogo cacheado, mesmo padrão de `stores` (tipos de canal Bling): `PaymentMethod` (`src/modules/sales/orders/payment_method/`) guarda `id_system` (id numérico da forma de pagamento na Bling, unique), `description` (texto livre configurado por conta Bling, ex. "Mercado Pago", "Pix" — não é enum fechado), `payment_type` (código numérico `tipoPagamento` da Bling, tabela de valores não confirmada no código) e `raw_payload`. Resolvido por `BlingOrderService.resolvePaymentMethod` (mesmo padrão cache-then-fetch de `resolveInvoiceId`: busca local por `id_system` antes de bater em `/formas-pagamentos/{id}`), chamado nos dois fluxos de sync de pedido (create/update) ao lado de `resolveInvoiceId`. Consumido pelo módulo PDV Management (`.claude/entities/pdv-sales-request/index.md`) pra comparar forma de pagamento × comprovante — comparação em si ainda não implementada (Passo B).
