...
`extractFromText`/`extractFromInlineData` passam pelo `withRetry` privado: até 4 retries com backoff linear (1s, 2s, 3s, 4s), só pra erro com `status` 500/503 (visto em produção como picos de demanda passageiros da API do Gemini) — qualquer outro status (404, 400, auth) propaga na primeira tentativa. 4 (não 2) porque a análise roda em background (ver seção "Análise assíncrona" em `.claude/entities/pdv-sales-request/index.md`) e não bloqueia mais a resposta HTTP — pode ser mais tolerante sem piorar a UX, importante com várias lojas subindo comprovante ao mesmo tempo.
