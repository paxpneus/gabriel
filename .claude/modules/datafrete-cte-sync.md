...
### Agrupamento por transportador (`TRANSPORTADOR_NAO_ENCONTRADO`/712)

A dedup padrão de `integration_errors` é por `(entity, type, integrations_id, internal_id, external_id)` (ver `.claude/entities/integration-error.md`), e nos dois catches acima `internal_id` normalmente é `cte.id` — cada CT-e é uma linha própria. Isso é errado especificamente pro erro 712: o transportador emissor não cadastrado na conta Datafrete afeta **todo** CT-e daquele transportador, então antes do fix cada CT-e não sincronizado gerava sua própria linha (chegou a 300+ linhas em produção pro mesmo CNPJ).

`buildCteDatafreteErrorLogFields(error, cte)` (`transporters/data-frete/helpers/cte-error-log.helper.ts`), chamada nos dois catches antes de montar o `integrationLoggerService.log(...)`, decide os campos de dedup: se `isDatafreteTransportadorNaoEncontrado(error)` (código 712) e `extractDatafreteTransportadorCnpj(error)` conseguir extrair o CNPJ da `mensagem` crua (regex sobre o padrão `'<cnpj>'` que a Datafrete sempre inclui nela), usa `internalId: null` + `externalId: <cnpj>` + `reference: <cnpj>` — todo CT-e do mesmo transportador cai na mesma linha (`occurrences` incrementa a cada novo CT-e afetado, sem criar linha nova). Qualquer outro erro (ou 712 sem CNPJ extraível, fallback defensivo) mantém o comportamento antigo: `internalId: cte.id` + `reference: cte.xml_key/número`, uma linha por CT-e.
...
## `setupCteAssociations()` — nunca chamada, e quebrada

`src/config/sequelize-associations.ts` tem uma função `setupCteAssociations()` (13 `Cte.belongsTo(...)`, papéis issuer/sender/recipient/dispatcher/receiver/taker contra `Transporter`/`Customer`/`Supplier`/`UnitBusiness`) que **não é chamada em lugar nenhum** — só `setupAssociations()` roda em `server.ts`. Achado ao investigar um bug não relacionado (associações do módulo PDV Management, ver `.claude/entities/pdv-sales-request/index.md`) que estavam por engano dentro dessa mesma função órfã. Se alguém tentar chamá-la, ela quebra: todo `targetKey: "tax_id"` nela está errado — `Transporter` usa `cnpj`, `Customer`/`Supplier` usam `document`, nenhum tem `tax_id` (o comentário original já admitia isso: `// Assumindo que tax_id é a coluna de CNPJ na model`). O fluxo real de CT-e descrito acima nunca precisou disso — resolve tomador/emitente por CNPJ direto em coluna (`taker_tax_id`), sem `include`. Corrigir e ligar essa função é trabalho separado, pendente.
