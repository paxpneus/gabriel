...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    // Busca todas as invoices que possuem source_payload com contato.id (ou fornecedor.id)
    ...
    const invoices = await queryInterface.sequelize.query(
      ...
      FROM invoices
      WHERE (
          source_payload -> 'contato' ->> 'id' IS NOT NULL
          OR source_payload -> 'fornecedor' ->> 'id' IS NOT NULL
        )
    ...
    console.log(`[populate-invoice-supplier-id-v3] ${invoices.length} invoices a processar.`);

    // Cache em memória para evitar buscar/criar o mesmo fornecedor várias vezes
    const supplierCache = new Map(); // key: id_system -> supplier id (uuid)
    ...
    for (const invoice of invoices) {
      ...
      // Prioriza contato.id (formato real do Bling), cai para fornecedor.id se existir
      const rawId = contato?.id ?? fornecedor?.id;
        ...
        if (existingById.length > 0) {
          ...
          if (existingByDoc.length > 0) {
            ...
            if (existingByName.length > 0) {
              ...
              // 4. Não encontrou -> cria novo supplier
