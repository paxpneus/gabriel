...
> {
  ...
  t: Transaction,
): Promise<{ batchInvoices: ExpeditionBatchInvoice[]; volumesAdded: number }> {
  ...
  for (const item of allItems) {
    ...
    quantityToAddByProductId.set(item.product_id, current + item.quantity_expected);
  }
  ...
  const quantityByKey = new Map<string, { batchItemId: string; batchInvoiceId: string; quantity: number }>();
  ...
  for (const [key, { batchItemId, batchInvoiceId, quantity }] of quantityByKey) {
