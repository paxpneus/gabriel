...
> {
  ...
  async removeBatchInvoice(id: string, externalTransaction?: Transaction): Promise<void> {
  const run = async (t: Transaction) => {
    const batchInvoice = (await this.findById(id, { transaction: t })) as ExpeditionBatchInvoice;
    ...
    const invoiceFound = await invoiceService.findById(batchInvoice.invoice_id, { transaction: t });
