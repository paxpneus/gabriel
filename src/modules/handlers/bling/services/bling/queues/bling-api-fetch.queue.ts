...
export class BlingApiFetchQueue extends BaseQueueService<ApiFetchJobPayload> {
  ...
  async process(job: Job<ApiFetchJobPayload>): Promise<void> {
    ...
    console.log(`[BLING_API_FETCH] Processando ${resource}.${action} | blingId=${apiFetch.blingId} | eventId=${eventId}`);
    ...
    try {
      switch (resource) {
        ...
        default:
          console.warn(`[BLING_API_FETCH] Sem handler de fetch para resource=${resource}`);
      }
    } catch (error: any) {
      console.error(`[BLING_API_FETCH] Erro ao processar job ${job.id}:`, error.message);
      throw error;
    }
  }
  ...
  private async fetchAndUpsertProduct(apiFetch: ApiFetchRequest): Promise<void> {
    ...
    if (!blingProduct?.codigo) {
      console.warn(`[BLING_API_FETCH] Produto ${apiFetch.blingId} sem SKU na Bling. Ignorado.`);
      return;
    }
 ...
 private async fetchAndUpsertProductSupplier(apiFetch: ApiFetchRequest): Promise<void> {
  ...
  private async fetchAndUpsertInvoice(
    ...
    const invoiceType: 'INCOMING' | 'OUTGOING' = nf.tipo === 1 ? 'INCOMING' : 'OUTGOING';
    ...
    const customerDoc = nf.contato ? (nf.destinatario?.cnpj ?? nf.destinatario?.cpf ?? '') : '';
    ...
    console.log(`[BLING_API_FETCH] ${nf.itens.length} item(ns) upsertado(s) para invoice ${nf.id}`);
  }
