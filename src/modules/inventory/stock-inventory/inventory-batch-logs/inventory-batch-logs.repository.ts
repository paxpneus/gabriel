...
export class InventoryBatchLogsRepository extends BaseRepository<InventoryBatchLogs> {
  ...
  const userDivergency =
    allLogs.length === 2
      ? Math.abs(Number(allLogs[0].quantity_read) - Number(allLogs[1].quantity_read))
      : 0;
  ...
  const newStatus = !hasUserDivergency && anyUserReadEnough ? "FINISHED" : "PENDING";
...
async syncDivergencyParent(
  ...
  const childItem = await InventoryBatchItems.findByPk(itemId, { transaction: t });
  if (childItem?.status !== "FINISHED") return;

  const parentItem = await InventoryBatchItems.findOne({
    where: { ean: productcode, inventory_batch_id: parentBatchId, stock_id: stockId },
    transaction: t,
    lock: t.LOCK.UPDATE,
  });
  ...
  if (existingParentLog) {
    await existingParentLog.update({ quantity_read: newUserRead }, { transaction: t });
  } else {
