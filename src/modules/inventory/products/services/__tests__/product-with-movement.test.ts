...
describe("StockMovementService", () => {
  ...
  beforeEach(() => {
    ...
    (service as any).repository = {
      findLastPurchaseEntries: jest.fn().mockResolvedValue(mockLastPurchaseEntries),
      findLastMovementsByProducts: jest.fn().mockResolvedValue(mockLastMovement),
    };
  });

  describe("getLastPurchaseEntries", () => {
    it("delega pro repository com os mesmos parâmetros e devolve o Map sem alterar", async () => {
      ...
      const result = await service.getLastPurchaseEntries(
        [PRODUCT_ID],
        UNIT_BUSINESS_ID,
        asOfDate,
        2,
      ...
      expect((service as any).repository.findLastPurchaseEntries).toHaveBeenCalledWith(
        [PRODUCT_ID],
        UNIT_BUSINESS_ID,
        asOfDate,
        2,
