...
async function waitForQueuesToDrain(label: string) {
  ...
  while (true) {
    ...
    console.log(
      `  ↻ Jobs pendentes: ${totalPending} — checando em ${QUEUE_POLL_MS / 1000}s...`,
    );
...
const STEPS = [
  ...
  { key: "sellers", label: "👤  Vendedores", fn: () => migrateSellers() },
  {
    key: "product_suppliers",
    label: "🔗  Produto-Fornecedor",
    fn: () => migrateProductSuppliers(),
  },
  { key: "stocks", label: "📊  Estoques", fn: () => migrateStocks() },
  {
    key: "nfe_out",
    label: "🧾  NF-e Saída  (tipo 1)",
    fn: () => migrateInvoices("NF-e", 1),
  },
  {
    key: "nfe_in",
    label: "🧾  NF-e Entrada (tipo 0)",
    fn: () => migrateInvoices("NF-e", 0),
  },
  {
    key: "nfe_cancelled",
    label: "🚫  NF-e Canceladas",
    fn: () => migrateCancelledInvoices("NF-e"),
  },
  { key: "nfce", label: "🧾  NFC-e", fn: () => migrateInvoices("NFC-e", 1) },
  { key: "orders", label: "🛒  Pedidos", fn: () => migrateOrders() },
] as const;
