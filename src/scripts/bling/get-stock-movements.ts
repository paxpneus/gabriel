...
async function doAutoLogin(page: Page): Promise<boolean> {
 ...
 // usam só o que é estável nas duas versões: o id do usuário e o texto do
 // botão — e esperam o elemento ficar visível antes de interagir, já que a
 // página é renderizada via JS e pode não estar pronta logo após o
 // domcontentloaded.
 //
 ...
 // "Não permitido") usam pra pontuar automação, independente da senha estar
 // certa (confirmado 2026-09-17: login manual com as mesmas credenciais
 // funciona, o mesmo preenchimento via `.fill()` + `.click()` imediato não).
 // `pressSequentially` dispara teclas de verdade com delay entre elas;
 // as pausas extras abaixo imitam o tempo de "tab" entre campos e de
 // conferir antes de clicar.
 try {
