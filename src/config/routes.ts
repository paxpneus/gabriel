...
function loadModules(basePath: string) {
  for (const entry of fs.readdirSync(basePath)) {
    ...
    if (routeFile) {
      const { default: moduleRouter } = require(
        path.join(entryPath, routeFile),
      );
      ...
      if (modelFile) {
        try {
          ...
          if (raw) {
            ROUTE_TO_TABLE[entry] =
              typeof raw === "string" ? raw : raw.tableName;
          }
        } catch {
      ...
      console.log(
        `Rota: /api/${entry} → tabela: ${ROUTE_TO_TABLE[entry] ?? "(sem model)"}`,
      );
    } else {
...
export function resolveEntityFromRoute(originalUrl: string): string | null {
  ...
  for (const pattern of Object.keys(CUSTOM_ROUTE_MAP)) {
    if (cleanUrl.includes(pattern)) return CUSTOM_ROUTE_MAP[pattern];
  }
